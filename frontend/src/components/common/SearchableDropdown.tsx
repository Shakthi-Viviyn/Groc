    ...
    return (
        <div className="flex gap-2">
            ...
            <div className="relative">
                <input type="text" name="name" className="bg-slate-200 rounded-md px-2 py-1 dpdown-input-effect" onFocus={handleDropdownVisibility} onBlur={handleDropdownVisibility} value={searchText} onChange={handleTextChange} autoComplete="off"/>
                { showDropdown && (options.length > 0) && <div className="absolute top-[120%] bg-slate-200 w-full rounded-lg shadow-lg z-50 max-h-[500%] overflow-y-scroll border-2 border-slate-500">
                    {options.map((option) => {
                        return (
                            <div className="p-2 hover:bg-slate-300 cursor-pointer" key={option.id}>
                                <p> {option[nameFieldKey]}</p>
                            </div>
                        )
                    })}
                </div> }
